...
export const JourneyNavigation: React.FC<JourneyNavigationProps> = ({
  ...
  // Exact vertical position calculation for radar indicator node:
  // Item 0: 0px
  // Item 1: 48px
  // Item 2: 96px (DISCIPLINA)
  // Item 3: 144px
  // Item 4: 192px
  const STEP_HEIGHT = 48;
  const indicatorTop = effectiveIndex * STEP_HEIGHT + 6;

  return (
    <div className="relative flex flex-col z-30" id="hero-journey-nav">
      
      {/* ─── DESKTOP VERTICAL NAVIGATION ─── */}
      <div className="hidden lg:flex items-center gap-6 relative">
        
        {/* Track Column for the Moving Golden Radar Node */}
        <div className="relative w-7 h-[230px] flex items-start justify-center flex-shrink-0">
          ...
          {/* Smoothly Moving Active Radar Target Node */}
          <div
            className="absolute left-0 w-7 h-7 flex items-center justify-center transition-all duration-350 ease-out pointer-events-none"
            style={{
              top: `${indicatorTop}px`,
            }}
          >
            ...
            <div
              className={`w-6 h-6 border border-[#E5AD08] rounded-full flex items-center justify-center transition-all duration-300 ${
                hoveredTarget
            ...
            >
              ...
              <div
                className={`w-2 h-2 bg-[#E5AD08] rounded-full transition-transform duration-200 ${
          ...
          {JOURNEY_NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className={`h-[36px] flex items-center gap-3 text-left transition-all duration-200 cursor-pointer select-none rounded-sm px-2.5 py-1 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#E5AD08] ${
                  isActive
                    ? 'opacity-100'
              ...
              >
                ...
                {/* Label */}
                <span
                  className={`font-semibold tracking-[0.18em] transition-all duration-200 ${
                    isActive
          ...
          {JOURNEY_NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                onClick={() => onSelect(item.target)}
                aria-label={item.ariaLabel}
                className={`flex-shrink-0 px-3 py-1.5 border text-[11px] font-mono tracking-wider flex items-center gap-1.5 transition-all duration-200 cursor-pointer ${
                  isActive
